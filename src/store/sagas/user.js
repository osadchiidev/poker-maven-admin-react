...
export function* loginRequest(action) {
    try {
        ...
        const { token, RealName, Avatar, Balance, Player, success, msg } = res.data;
        if (msg) {
            if (success)
                toast.success(msg);
            else
        ...
        if (success) {
            ...
            sessionStorage.setItem("user", JSON.stringify({ RealName: RealName, Avatar: Avatar, Balance: Balance, Player: Player }));
            yield call(axios.setToken, token);
            yield put(loginSuccess({ RealName, Avatar, Balance }));
